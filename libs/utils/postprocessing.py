# modify from https://github.com/happyharrycn/actionformer_release/blob/main/libs/utils/postprocessing.py
...
def postprocess_results(results, cls_score_file, num_pred=200, topk=2):
    ...
    for vid, result in results.items():
        ...
        new_pred_score = np.sqrt(
            topk_cls_score[:, None] @ pred_score[None, :]).flatten()
        new_pred_segment = np.tile(pred_segment, (topk, 1))
        new_pred_label = np.tile(
            topk_cls_idx[:, None], (1, num_segs)).flatten()
